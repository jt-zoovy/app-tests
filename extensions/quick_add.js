...
//these are going the way of the do do, in favor of app events. new extensions should have few (if any) actions.
		a : {
			...
			addItemToCart : function($form)	{
				...
				app.ext.store_product.u.handleAddToCart($form,{'callback':function(rd){
					...
					if(app.model.responseHasErrors(rd)){
						...
						$QC.anycontent({'templateID':'',datapointer:app.data['appProductGet|'+$("input[name='sku']",$form).val()]})
						}
					}});
			...
			showQuickCart : function()	{
				app.u.dump("BEGIN cart_quickadd.a.showQuickCart");
				var $QC = $("#quickCart");
				if($QC.length)	{}
				else	{
					$QC = $("<div \/>",{'id':'quickCart'}).css({'display':'none','position':'absolute','top':100,'right':-200,'z-index':10000,'height':100,'width':220}).addClass('ui-widget ui-widget-content ui-corner-left');
					$QC.appendTo('body');
					}
